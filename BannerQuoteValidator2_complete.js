...
const DEFAULT_ZIP_CODE = process.env.DEFAULT_ZIP_CODE || '43215';
const DEFAULT_ACTION_TIMEOUT_MS = 10000;
const DEFAULT_PAGE_STABILIZE_MS = 500;
...
function generateCsvReport(results, outputPath, urls = [], deviceName = null) {
  const rows = [
    ['Validation URL', 'Category', 'Target', 'Status', 'Redirect URL', 'Target Window', 'Details']
  ];
  ...
  for (const result of filteredResults) {
    ...
    const redirectUrl = result.details?.redirectUrl || result.details?.finalUrl || '';
    const targetWindow = result.details?.targetWindow || (result.details?.outcome === 'popup' ? 'new-window' : (result.details?.outcome === 'same-tab' ? 'same-window' : ''));
    rows.push([
      escapeCsv(result.url || ''),
      escapeCsv(result.category),
      escapeCsv(result.target),
      escapeCsv(status),
      escapeCsv(redirectUrl),
      escapeCsv(targetWindow),
      escapeCsv(JSON.stringify(result.details).replace(/,/g, ';'))
    ]);
  }
      ...
      <table>
        <thead>
          <tr>
            ...
            <th style="width:180px;">Redirect URL</th>
            <th style="width:120px;">Target Window</th>
        ...
        <tbody>
          ${displayResults.map(result => {
            ...
            let statusText = '✅ PASS';
            const redirectUrl = result.details?.redirectUrl || result.details?.finalUrl || result.details?.popupUrl || result.details?.navigatedTo || '';
            const targetWindow = result.details?.targetWindow || (result.details?.outcome === 'popup' ? 'new-window' : (result.details?.outcome === 'same-tab' ? 'same-window' : ''));
            ...
            <tr class="${rowClass}">
              ...
              <td>${escapeHtml(redirectUrl)}</td>
              <td>${escapeHtml(targetWindow)}</td>
...
async function waitForPageReady(page, waitUntil = 'domcontentloaded', timeout = 15000, stabilizationMs = DEFAULT_PAGE_STABILIZE_MS) {
  await page.waitForLoadState(waitUntil, { timeout }).catch(() => {});
  if (stabilizationMs > 0) {
    await wait(stabilizationMs, 'Allowing page to stabilize');
  }
}

async function observeActionOutcome(page, action, beforeUrl = null, timeout = DEFAULT_ACTION_TIMEOUT_MS) {
  ...
  if (popup) {
    ...
    await wait(1000, 'Waiting for the resulting page state');
...
async function resetToHomePage(page, baseUrl) {
  ...
  if (normalize(currentUrl) !== normalize(baseUrl)) {
    ...
    await waitForPageReady(page, 'domcontentloaded', 30000, 2000);
    return true;
  }
  return false;
}
...
async function findBannerQuoteDropdown(page) {
  ...
  const bannerSelectors = [
    ...
    'select#custom-account-dropdown',
    'select[name="custom-account-dropdown"]',
    'select[name*="account"]',
    'select[name*="product"]',
...
async function checkForZipError(page) {
  ...
  try {
    ...
    for (const selector of genericErrorSelectors) {
      ...
      if (await element.count() > 0 && await element.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        if (text) {
          console.log(`  ✅ Found inline form error: "${text}"`);
          return { text, type: 'inline-form-error' };
        }
      }
    }
  } catch (error) {
...
async function findStartQuoteButton(page, option, isSeeAllInsurance = false, container = null) {
  ...
  const prioritySelectors = [
    ...
    '#LifeQuoteForm > div:nth-child(2) > div > a',
    '#LifeQuoteForm > div > div > a',
    '#PetQuoteForm > div:nth-child(2) > div > a',
    '#PetQuoteForm > div > div > a',
    '.btn-start-quote',
    '.cta-button',
  ...
  const isPet = option && (option.value === 'pet' || option.text?.toLowerCase().includes('pet insurance'));
...
async function processBannerQuoteOption(page, option, baseUrl, dropdownType = 'banner', zipCode = '43215', isErrorPage = false) {
  ...
  if (dropdownType === 'banner' && zipCode) {
  ...
  await wait(750, 'Page stabilization');
  ...
  await wait(750, 'Option selected - waiting for dynamic content to load');

  const selectedValue = (option.value || '').trim();
  const isDirectNavigationOption = /^https?:\/\//i.test(selectedValue) || selectedValue.startsWith('/');
  if (isDirectNavigationOption) {
    const targetUrl = /^https?:\/\//i.test(selectedValue)
      ? selectedValue
      : new URL(selectedValue, baseUrl).toString();
    console.log(`  → Option points to a direct URL, navigating to: ${targetUrl}`);
    const beforeUrl = page.url();
    await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
    await wait(2000, 'Waiting for direct navigation target to load');
    ...
    return {
      ...
      requiresZip: false,
      zipCode: null,
      outcome: finalUrl !== beforeUrl ? 'same-tab' : 'no-change',
      finalUrl: finalUrl,
      bannerPresent: hasBanner,
      isSeeAllInsurance: false,
      navigationSuccess: finalUrl !== beforeUrl,
      buttonFoundBy: 'direct-url-navigation',
      negativeTests: [],
  ...
  if (urlAfterSelection !== baseUrl && !urlAfterSelection.includes('about:blank')) {
    ...
    return {
      ...
      option: option.text,
      optionValue: option.value,
      requiresZip: dropdownType === 'banner' && Boolean(zipCode),
      zipCode: dropdownType === 'banner' ? zipCode : null,
      outcome: 'same-tab',
  ...
  if (isRV) {
    const phoneNumberElement = await page.locator('strong:has-text("1-877-"), div:has-text("Call:")').first();
    const agentLinkElement = await page.locator('a:has-text("Find an agent"), a[href*="agency.nationwide.com"]').first();
    ...
    if (await phoneNumberElement.count() > 0 && await phoneNumberElement.isVisible()) {
      phoneNumberText = (await phoneNumberElement.textContent().catch(() => '') || '').trim();
      if (!phoneNumberText || !phoneNumberText.includes('1-877-')) {
        validationPassed = false;
        validationMessages.push('Phone number text not found or invalid');
      }
    } else {
      validationPassed = false;
      validationMessages.push('RV phone number not displayed');
    }

    if (await agentLinkElement.count() > 0 && await agentLinkElement.isVisible().catch(() => false)) {
      const href = await agentLinkElement.getAttribute('href').catch(() => '');
      const text = (await agentLinkElement.textContent().catch(() => '') || '').trim();
      agentLink = { text, url: href };
      if (!text || !href) {
        validationPassed = false;
        validationMessages.push('Find an agent link text or URL missing');
      }
    } else {
      validationPassed = false;
      validationMessages.push('Find an agent link not displayed');
    }

    console.log('  ℹ RV insurance value detected; validating phone and agent link display');
    const currentUrl = page.url();
    return {
      success: validationPassed,
      reason: validationPassed ? null : 'RV display validation failed',
      validationMessages,
      option: option.text,
      optionValue: option.value,
      requiresZip: false,
  ...
  if (dropdownType === 'custom-tri-promo') {
    ...
    const beforeUrl = page.url();
    const actionResult = await observeActionOutcome(page, async () => {
      if (goButton) {
    ...
    const finalUrl = actionResult.finalUrl || page.url();
    if (finalUrl) {
      console.log(`  → Resulting page: ${finalUrl}`);
    }
    const actionOccurred = actionResult.popupOpened || actionResult.navigationDetected;
    return {
      ...
      bannerPresent: hasBanner,
      isSeeAllInsurance: false,
      navigationSuccess: actionResult.navigationDetected,
  ...
  if (isPropertyPage) {
    ...
    await wait(1000, 'Extra wait for property page');
  }
  ...
  if (zipField && !isSeeAllInsurance) {
    ...
    if (startButton) {
      ...
      await wait(1000, 'Waiting for response');
      const currentUrl = page.url();
      ...
      await resetToHomePage(page, baseUrl);
      await wait(500, 'Page reset after negative test');
      ...
      await wait(250, 'Option re-selected');
    }
    ...
    if (zipFieldRefreshed) {
      ...
      await wait(500, 'Waiting for validation to complete');
      ...
      const popupOpened = actionResult.popupOpened;
      const navigationDetected = actionResult.navigationDetected;
      const finalUrl = actionResult.finalUrl || page.url();
      ...
      if (!actionOccurred) {
        inlineError = await checkForZipError(page);
        testSuccess = false;
      ...
      const result = {
        success: testSuccess,
        reason: reason,
        inlineError: inlineError ? { text: inlineError.text, type: inlineError.type } : null,
        option: option.text,
        optionValue: option.value,
    ...
    await wait(500, 'Waiting for ZIP field to appear');
    ...
    if (submitButton) {
      console.log('  → Clicking the resolved CTA button');
      await submitButton.click();
    } else {
      ...
      for (const fallbackSelector of fallbackSelectors) {
        const fallbackButton = page.locator(fallbackSelector).first();
        if (await fallbackButton.count() > 0 && await fallbackButton.isVisible({ timeout: 2000 }).catch(() => false)) {
          console.log(`  → Clicking fallback CTA using: ${fallbackSelector}`);
...
async function processServicingOption(page, option, baseUrl, isErrorPage = false) {
  ...
  await wait(250, 'Option selected');
...
async function runTestForUrl(baseUrl, page) {
  ...
  try {
    ...
    recordResult('Step 4', 'Banner Check', 'banner-level2-component-link-content', hasBanner, {
      present: hasBanner,
      status: hasBanner ? 'present' : 'not-present',
      message: hasBanner ? 'Component present' : 'Component not present'
    });
    ...
    if (bannerOptions.length > 0 && !isErrorPage) {
      ...
      for (let i = 0; i < bannerOptions.length; i++) {
        ...
        try {
          ...
          await wait(1000, 'Waiting before next banner option');
        } catch (error) {
    ...
    if (promoOptions.length > 0 && !isErrorPage) {
      ...
      for (let i = 0; i < promoOptions.length; i++) {
        ...
        try {
          ...
          await wait(1000, 'Waiting before next promo option');
        } catch (error) {
          console.log(`  ❌ Error processing ${option.text}: ${error.message}`);
          recordResult('Step 6b', 'Promo Quote', option.text, false, {
            error: error.message
    ...
    if (promoOptions.length > 0) {
      ...
      await page.goto(baseUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
      await wait(1000, 'Page loaded before servicing tests');
        ...
        if (servicingOptions.length > 0) {
          for (let i = 0; i < servicingOptions.length; i++) {
            ...
            try {
              ...
              await wait(1000, 'Waiting before next servicing option');
            } catch (error) {
              console.log(`  ❌ Error processing ${option.text}: ${error.message}`);
              recordResult('Step 7', 'Servicing', option.text, false, {
                error: error.message
